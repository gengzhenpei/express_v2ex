...
<template>
	<div class="">
		<!--{{articleList}}-->
		<div class="recommend ba" v-for="item in articleList" v-if="item.id != $route.params.id" @click="goArticle(item)">
        ...
        <a>{{ item.title }}</a>
			...
			<div class="foot wid80 f-left">
				<a v-if="item.classify" v-for="items in markSplit(item.classify)" class="classify">
					{{ items }}
				</a>
				<span>
